...
import net.mcreator.palworld.network.SubQuestCompleteButtonMessage;
...
public class SubQuestCompleteScreen extends AbstractContainerScreen<SubQuestCompleteMenu> {
	...
	protected void renderLabels(GuiGraphics guiGraphics, int mouseX, int mouseY) {
		...
		guiGraphics.drawString(this.font, Component.translatable("gui.palworld.sub_quest_complete.label_bosang"), 33, 45, -12829636, false);
		guiGraphics.drawString(this.font,
	...
	public void init() {
		super.init();
		button_surag = Button.builder(Component.translatable("gui.palworld.sub_quest_complete.button_surag"), e -> {
			if (AcceptApearProcedure.execute(entity)) {
				PacketDistributor.sendToServer(new SubQuestCompleteButtonMessage(0, x, y, z));
				SubQuestCompleteButtonMessage.handleButtonAction(entity, 0, x, y, z);
			}
		...
		button_geojeol = Button.builder(Component.translatable("gui.palworld.sub_quest_complete.button_geojeol"), e -> {
			if (AcceptApearProcedure.execute(entity)) {
				PacketDistributor.sendToServer(new SubQuestCompleteButtonMessage(1, x, y, z));
				SubQuestCompleteButtonMessage.handleButtonAction(entity, 1, x, y, z);
			}
		...
		button_dadgi = Button.builder(Component.translatable("gui.palworld.sub_quest_complete.button_dadgi"), e -> {
			if (QuestTapCloseButtonAppearProcedure.execute(entity)) {
				PacketDistributor.sendToServer(new SubQuestCompleteButtonMessage(2, x, y, z));
				SubQuestCompleteButtonMessage.handleButtonAction(entity, 2, x, y, z);
			}
		...
		button_bosanghoegdeug = Button.builder(Component.translatable("gui.palworld.sub_quest_complete.button_bosanghoegdeug"), e -> {
			if (RewardButtonAppearProcedure.execute(entity)) {
				PacketDistributor.sendToServer(new SubQuestCompleteButtonMessage(3, x, y, z));
				SubQuestCompleteButtonMessage.handleButtonAction(entity, 3, x, y, z);
			}
		}).bounds(this.leftPos + 36, this.topPos + 82, 103, 20).build(builder -> new Button(builder) {
			@Override
			public void renderWidget(GuiGraphics guiGraphics, int gx, int gy, float ticks) {
