...
if (world.dayTime()>=) {deg = 0;r = 1;i = 1;if (world instanceof ServerLevel _level) {
(entity instanceof LivingEntity _livEnt ? _livEnt.getMainHandItem():ItemStack.EMPTY).hurtAndBreak(1, _level, null, _stkprov -> {});
}for (int index0 = 0; index0<1; index0++) {for (int index1 = 0; index1<1; index1++) {for (int index2 = 0; index2<11; index2++) {world.addParticle((SimpleParticleType) (PalworldModParticleTypes.DELETED_MOD_ELEMENT.get()), (x-r*Math.sin(Math.toRadians(deg))), (y+i), (z+r*Math.cos(Math.toRadians(deg))), ((-0.1)*Math.sin(Math.toRadians(deg))), 0, (0.1*Math.cos(Math.toRadians(deg))));{
...
}deg = deg+36;}if (i==2||i==4) {deg = 18;}else{deg = 0;}r = r+1;}}if(entity instanceof LivingEntity _entity && !_entity.level().isClientSide())
_entity.addEffect(new MobEffectInstance(PalworldModMobEffects.DELETED_MOD_ELEMENT, 30, 1, false, false));}
}
}
